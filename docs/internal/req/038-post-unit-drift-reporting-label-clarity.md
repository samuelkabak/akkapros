...
- [ ] Given phonetizer front matter is emitted, when drift-reporting fields are
      named, then the names identify the quantity as post-unit drift.
- [ ] Given user-facing report headings or table labels expose the same values,
      when they are rendered, then the labels use `Post-unit drift` wording.
- [ ] Given the rename is applied, when the same input is processed, then the
      numeric values remain unchanged.
- [ ] Given the row-level flat file column remains named `drift`, when public
