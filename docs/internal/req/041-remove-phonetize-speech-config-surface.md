...
- [x] Given the approved grouped config surface is materialized, when removed
...
- [x] Given phonetize semantic verification is executed, when config relations
...
- [x] Given an old config file still contains
      ...
      accepted.
- [x] Given the phonetizer implementation is inspected, when active timing
...
- [x] Given user-facing configuration and phonetizer documentation is updated,
      ...
      `phonetize.process.timing_model.speech.pause_ratio`.
- [x] Given repository-owned demos and sample configs are updated, when
      phonetize timing examples are inspected, then they do not include a
      `speech:` section under `phonetize.process.timing_model`.
- [x] Given tests cover the removed contract, when config-surface and CLI-path
...
- [x] Given metrics outputs are inspected after this removal, when `WPM` and
