# Req Index

This index lists requirement documents. It is maintained by `scripts/update-indexes.py`.

- [033. Requirement: Phonetizer Pause Bands and Pause Metrics Reporting](033-phonetizer-pause-bands-and-pause-metrics-reporting.md) - Draft
- [032. Requirement: Phonetizer Intonation and Three-Pass Finalization](032-phonetizer-intonation-and-three-pass-finalization.md) - Implemented
- [031. Requirement: Phonetizer Phase 2 Syllable-Scoped Duration Realization](031-phonetizer-phase-2-syllable-scoped-duration-realization.md) - Implemented
- [030. Requirement: Phone/Ophone-Only Metrics and Interval Rhythm Computation](030-phone-ophone-only-metrics-and-interval-rhythm-computation.md) - Implemented
- [029. Requirement: Stage Config Run/Process Separation](029-stage-config-run-process-separation-and-common-outdir-removal.md) - Implemented
- [028. Requirement: Effective Runtime Config Object, Path Overrides, and Program-Scoped Help](028-effective-runtime-config-object-path-overrides-and-program-scoped-help.md) - Implemented
- [027. Requirement: Phonetize Config Semantic Invariants for Shared Verification](027-phonetize-config-semantic-invariants-for-shared-verification.md) - Implemented
- [026. Requirement: Stability-First Phonetizer Timing Control and Baseline Validation](026-stability-first-phonetizer-timing-control-and-baseline-validation.md) - Implemented
- [025. Requirement: Two-Phase Phonetizer Structure and Duration Pipeline](025-two-phase-phonetizer-structure-and-duration-pipeline.md) - Draft
- [024. Requirement: Replacement of timing model](024-replacement-of-timing-model.md) - Implemented
- [023. Requirement: Mandatory Prefix, Library-Named Config Sections, and Inherited Syllabify Settings](023-mandatory-prefix-library-named-config-sections-and-inherited-punctuation-settings.md) - Implemented
- [022. Requirement: Package-Wide YAML Config and Confwriter CLI](022-package-wide-yaml-config-and-confwriter.md) - Implemented
- [021. Requirement: Separate Hiatus Marker for Word-Initial Vowel Hiatus](021-separate-hiatus-marker-for-word-initial-vowel-hiatus.md) - Implemented
- [020. Requirement: Configurable Merge-Connector Printing](020-configurable-merge-connector-printing.md) - Implemented
- [019. Requirement: Prosody Mora Mode Selection](019-prosody-mora-mode-selection.md) - Implemented
- [018. Requirement: Minimize Path Disclosure in Metrics Artifacts](018-minimize-path-disclosure-in-metrics-artifacts.md) - Implemented
- [017. Requirement: Frontmatter Minimization and Source-Flexible Pipeline Inputs](017-frontmatter-minimization-and-source-flexible-pipeline-inputs.md) - Implemented
- [016. Requirement: Standardized CLI Logging and Console Options](016-standardized-cli-logging-and-console-options.md) - Implemented
- [015. Requirement: Frontmatter-Derived Word Indicators in Metrics](015-frontmatter-derived-word-indicators-in-metrics.md) - Implemented
- [014. Requirement: Remove Metrics CSV Output](014-remove-metrics-csv-output.md) - Implemented
- [013. Requirement: CLI File Front Matter and Metadata Propagation](013-cli-file-front-matter-and-metadata-propagation.md) - Implemented
- [012. Requirement: Metrics Output Structure and Layout](012-metrics-output-structure-and-layout.md) - Implemented
- [011. Requirement: Strict Punctuation Whitelist with CLI Extensions](011-punctuation-whitelist-and-cli-extension.md) - Implemented
- [010. Requirement: Built-in Self-Tests and CLI Test Infrastructure](010-built-in-self-tests-and-test-infrastructure.md) - Implemented
- [009. Requirement: Phonological Research Model and Corpus Scope](009-phonological-research-model-and-corpus-scope.md) - Implemented
- [008. Requirement: Accentuation Terminology Rename (CR-004 Migration)](008-accentuation-terminology-rename.md) - Implemented
- [007. Requirement: Full Pipeline Orchestration (fullprosmaker)](007-full-pipeline-orchestration.md) - Implemented
- [006. Requirement: Diphone Recording Script Generator (phoneprep)](006-diphone-recording-script-generator.md) - Implemented
- [005. Requirement: Multi-Format Printer Output](005-multi-format-printer-output.md) - Implemented
- [004. Requirement: Rhythmic Metrics Computation](004-metrics-computation.md) - Implemented
- [003. Requirement: Bimoraic Prosody Realization Algorithm](003-bimoraic-prosody-realization-algorithm.md) - Implemented
- [002. Requirement: Akkadian Syllabification](002-syllabification.md) - Implemented
- [001. Requirement: eBL ATF Parsing and Normalization](001-ebl-atf-parsing-and-normalization.md) - Implemented
