...
- Test self-sufficiency rule: tests must use inputs, fixtures, and configuration reserved to tests. Any file or config needed to run a test must be either hardcoded in the test itself or stored under `tests/`. Tests must not depend on mutable artifacts from `demo/`, `outputs/`, `tmp/`, user work folders, or any other uncontrolled location whose contents may change outside the test contract.
...
- When writing or updating tests, keep the test self-sufficient: use hardcoded strings or fixtures/configs stored under `tests/`, not files from `demo/`, `outputs/`, `tmp/`, or other uncontrolled folders.
...
- **Governance quick reference:** `docs/internal/GUIDELINES.md` — compressed summary of all governance rules. Read this first before any governance work.
- **Code index:** `docs/internal/code-index/README.md` — usage and maintenance instructions for the code index.
  - `docs/internal/code-index/module-tags.yaml` — domain tags, coupling levels, test markers, and logical sections for each module.
  - `docs/internal/code-index/*_callgraph.mmd` — Mermaid call graphs for each CLI program (generated by `python scripts/gen_code_graph.py`).
