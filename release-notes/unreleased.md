...
- Escape delimiter migration note (CR-003): pipeline escape markers changed from `‹...›` to `⟦...⟧`; downstream parsers should migrate by replacing `‹`→`⟦` and `›`→`⟧` where used as escape markers.
